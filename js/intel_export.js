...
═══════════════════════════════════════════════`;
  ...
  async function searchCharities(name) {
    if (!name || name.length < 3) return [];
    try {
      // Charity Commission Register of Charities API (requires Ocp-Apim-Subscription-Key)
      const apiKey = window.CHARITY_API_KEY || "";
      const headers = {};
      if (apiKey) headers["Ocp-Apim-Subscription-Key"] = apiKey;

      const url = `https://api.charitycommission.gov.uk/register/api/SearchCharities?SearchTerms=${encodeURIComponent(name)}&SubsidiaryNumber=0&PageNumber=1&PageSize=10`;
      const resp = await fetch(url, { headers });
      if (!resp.ok) {
        // Fallback: try CharityBase open API (no key needed)
        return await searchCharitiesFallback(name);
      }
      const data = await resp.json();
      const items = data?.Charities || data || [];
  ...
  async function searchCharitiesFallback(name) {
    try {
      // CharityBase open GraphQL API (free, no key)
      const query = `{ CHC { getCharities(filters: { search: "${name.replace(/"/g, '\\"')}" }) { count list(limit: 10) { names { value primary } activities geo { postcode } registrations { registrationDate removalDate } } } } }`;
      const resp = await fetch("https://charitybase.uk/api/graphql", {
        method: "POST",
